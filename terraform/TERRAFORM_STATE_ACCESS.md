...
- Run locally with `az login`; run in GitHub Actions with `azure/login@v2` + UAMI federated credential (`repo:<YOUR-ORG>/<YOUR-REPO>:ref:refs/heads/main`).
...
- Keep federated credential subject scoped to `repo:<YOUR-ORG>/<YOUR-REPO>:ref:refs/heads/main` (prod only).
